...
def create_account(req: schemas.CreateAccountRequest, eng: BankingEngine = Depends(engine_dep)):
    return {"result": eng.create_account(req.timestamp, req.account_id)}


@router.post("/deposit", response_model=schemas.IntResult)
def deposit(req: schemas.DepositRequest, eng: BankingEngine = Depends(engine_dep)):
    return {"result": eng.deposit(req.timestamp, req.account_id, req.amount)}


@router.post("/transfer", response_model=schemas.IntResult)
def transfer(req: schemas.TransferRequest, eng: BankingEngine = Depends(engine_dep)):
    return {
        "result": eng.transfer(
...
def pay(req: schemas.PayRequest, eng: BankingEngine = Depends(engine_dep)):
    return {"result": eng.pay(req.timestamp, req.account_id, req.amount)}


@router.post("/payment-status", response_model=schemas.StrResult)
def payment_status(req: schemas.PaymentStatusRequest, eng: BankingEngine = Depends(engine_dep)):
    return {
...
def top_spenders(req: schemas.TopSpendersRequest, eng: BankingEngine = Depends(engine_dep)):
...
def merge(req: schemas.MergeRequest, eng: BankingEngine = Depends(engine_dep)):
...
def balance_at(req: schemas.BalanceAtRequest, eng: BankingEngine = Depends(engine_dep)):
